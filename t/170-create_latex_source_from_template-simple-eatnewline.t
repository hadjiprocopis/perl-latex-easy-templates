...
my $template_dir = File::Spec->catdir($curdir, 'templates', 'simple-eatnewline');
...
for my $aprocessorname (sort keys %{$latterparams->{'processors'}}){
	...
	# check if we have newlines between abc and xyz, they must have been eaten!
	ok($content !~ /abc\s+xyz/, 'untemplate()'." : called for processor '$aprocessorname', and latex source (content of file '".$untemplate_ret->{'latex'}->{'filepath'}."') does not look to contain newlines between 'abc' and 'xyz'.") or BAIL_OUT($content."\n\nno see above latex source content.");
